...
def upload_rates(provider_id, product, rate):
    ...
    """
    # resolve the path to the shared /in directory
    # on EC2: set by run-e2e.sh to $STAGING_DIR/tests/e2e/in (compose bind mount)
    # locally: falls back to the in/ folder next to this test file
    in_dir = os.getenv("E2E_IN_DIR", os.path.join(os.path.dirname(__file__), "in"))
