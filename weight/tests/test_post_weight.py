...
@pytest.fixture
def client():
    """Create a test client and clean up transactions before each test."""
    app.config["TESTING"] = True
    with app.app_context():
        # Clean up any leftover test data
        Transaction.query.filter(Transaction.truck.like("TEST-%")).delete()
        # Add a known container for neto calculation tests
...
def test_out_closes_session(client):
    ...
    # Weigh in with known containers
    client.post("/weight", json={
        "direction": "in",
        "truck": "TEST-007",
        "weight": 15000,
